...
bool usb_cam::query_control(int control_id, v4l2_queryctrl &queryctrl)
{
    ...
    if (ioctl(m_fd, VIDIOC_QUERYCTRL, &queryctrl) == -1)
    {
        if (errno != EINVAL)
        {
            CERR_ENDL("Failed to query control (" << control_name << ", ID: " << control_id
                                                  << "): " << strerror(errno));
        }
        else
        {
...
bool usb_cam::is_format_supported(const std::string &supported_format, const std::string &requested_format)
{
