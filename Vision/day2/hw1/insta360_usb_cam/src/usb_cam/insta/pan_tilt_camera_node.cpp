...
rclcpp_lifecycle::node_interfaces::LifecycleNodeInterface::CallbackReturn PanTiltCamera::on_configure(
    const rclcpp_lifecycle::State &)
{
    ...
    for (const auto &supported_format : device_info.formats)
    {
        ...
        if (camera_->is_format_supported(supported_format, format))
        {
            ...
            for (const auto &res_info : device_info.resolution_info)
            {
                ...
                if (res_info.resolution.first * res_info.resolution.second >
    ...
    compressed_image_pub_ = this->create_publisher<sensor_msgs::msg::Image>(compressed_topic.c_str(), 1);
    camera_info_pub_ = this->create_publisher<sensor_msgs::msg::CameraInfo>(camera_info_topic.c_str(), 5);
    ...
    pan_tilt_status_pub_ =
        this->create_publisher<insta360_usb_cam::msg::PanTiltStatusMsgs>(pan_tilt_status_topic.c_str(), 5);
    pan_tilt_sub_ = this->create_subscription<insta360_usb_cam::msg::InstaPanTiltMsgs>(
        pan_tilt_topic.c_str(), 5, std::bind(&PanTiltCamera::pan_tilt_callback, this, std::placeholders::_1));
...
rclcpp_lifecycle::node_interfaces::LifecycleNodeInterface::CallbackReturn PanTiltCamera::on_activate(
    const rclcpp_lifecycle::State &)
{
...
cv::Mat PanTiltCamera::letterbox(const cv::Mat &src)
{
    ...
    if (1LL * sw * dh == 1LL * sh * dw)
    {                                           // 정수 교차곱: FP오차 없음 => 1LL(longlong 혹시 몰라서)
    ...
    // 2) 일반 케이스: 레터박스
    const double s = std::min(static_cast<double>(dw) / sw, static_cast<double>(dh) / sh);
...
rclcpp_lifecycle::node_interfaces::LifecycleNodeInterface::CallbackReturn PanTiltCamera::on_deactivate(
    const rclcpp_lifecycle::State &)
{
...
rclcpp_lifecycle::node_interfaces::LifecycleNodeInterface::CallbackReturn PanTiltCamera::on_shutdown(
    const rclcpp_lifecycle::State &state)
{
    RCLCPP_INFO(this->get_logger(), "Shutting down PanTiltCamera from state: %s", state.label().c_str());
...
void PanTiltCamera::get_param()
{
    ...
    distortion_coeffs = this->declare_parameter<std::vector<double>>(
        "distortion_coefficients.data", std::vector<double>(distortion_coeff_rows * distortion_coeff_cols, 0.0));
    ...
    rectification_matrix = this->declare_parameter<std::vector<double>>(
        "rectification_matrix.data", std::vector<double>(rectification_matrix_rows * rectification_matrix_cols, 0.0));
    ...
    projection_matrix = this->declare_parameter<std::vector<double>>(
        "projection_matrix.data", std::vector<double>(projection_matrix_rows * projection_matrix_cols, 0.0));
...
void PanTiltCamera::set_param(std::string parameter_name, int parameter_value)
{
    ...
    if (parameter_value != -1)
    {
        if (camera_->query_control(control_id, queryctrl))
        {
            ...
            {
                RCLCPP_WARN(this->get_logger(), "Failed to set %s to %d", parameter_name.c_str(), parameter_value);
            }
        }
        else
        {
            RCLCPP_WARN(this->get_logger(), "%s is not available on this device.", parameter_name.c_str());
        }
    }
    else
    {
...
void PanTiltCamera::set_param_auto(std::string parameter_name, std::string auto_parameter_name, int parameter_value,
